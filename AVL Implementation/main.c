 ...
 * -----------------------------
 *   frees memory of every AVL node and AVL inside the given list
 ...
 * ------------------------------
 *   stores student ID entered by user in a new AVL node
 *   in the AVL tree of the listnode
...
void option_enrol_student(CourseList* my_list)
{
    ...
    insert_avl(&targeted_course->students, prompt_int());
}

/*
 * Function: option_unenrol_student
 * --------------------------------
 *   deletes the AVL node where the student ID entered by user was stored
 *   from the AVL tree of the listnode
 *   which contains the course name entered by the user
 *
 *   my_list: address of the list which cointains the listnode
...
void option_unenrol_student(CourseList* my_list)
{
    ...
    targeted_course = search_course_node(my_list, prompt_string());
    printf("Enter Student ID: ");
    delete_avl(&targeted_course, prompt_int());
}
 ...
 *   counting every AVL node in the AVL tree of the listnode
 *
 ...
 *   prints the student ID(s) in the AVL nodes inside the AVL tree of that course
 *   in ascending order
 *
...
void print_student_list(CourseList* my_list)
{
    ...
    targeted_course = search_course_node(my_list, prompt_string());
    print_in_order_avl(&(targeted_course->students));
}
 ...
 *   searches for the AVL node containing student ID received from user input,
 *   in the AVL tree. If the student exists in that tree 
...
void print_course_list(CourseList* my_list)
{
    ...
    while (current != NULL)
    {
        if (find_avl(&(current->students), id) != NULL)
        {
            printf("%s \n", current->course);
        }
        current = current->next;
